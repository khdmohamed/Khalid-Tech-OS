# Client Context — Alsafwa Alarabia / Konooz

**Industry:** Retail / E-Commerce
**Primary Contact:** Tahir <tahir@alsafwahar.com>
**Contract Type:** Project-based (Techasus Arabia)  

## Systems in Use
- ERP: Business Central + LS Central
- E-Commerce: Shopify
- Logistics: Torod
- Compliance: ZATCA Phase 2

## Active Projects
- PRJ-006 — Return Flow & Checkout Completion (go-live blocker)
- PRJ-007 — Order Status Analysis (completed)
- Shopify–LS Central integration (sync, pricing, inventory)
- ZATCA-compliant tax-inclusive pricing
- Arabic RTL theme development
- Torod logistics integration
- Customer account pages, return/cancel flows

## Notes
- Pricing must be tax-inclusive (ZATCA requirement)
- Arabic RTL is mandatory, not optional
- GraphQL API used for Shopify side
- Integration partner delays have been a recurring issue
- Urgent go-live timeline — all pending items must be complete

## Contacts
| Role        | Name                 | Email                     |
| ----------- | -------------------- | ------------------------- |
| Client Lead | Tahir                | tahir@alsafwahar.com      |
| CEO Office  | CEO Office           | ceo.office@alsafwahar.com |
| Team        | Mohammed Al-Khunfari | mohammed@alsafwahar.com   |
| Team        | Ahmed Abdulhamid     | ahmed@alsafwahar.com      |
| Techasus PM | Abdallah Tawfik      | -                         |
